...
pub async fn fetch_one_regexword(
    ...
    match store.fetch_one(&id).await {
        Ok(Some(entity)) => {
            let view = from_states_to_view(entity, "regexword".to_string(), &ctx);
...
#[utoipa::path(
    tag = "regexword",
    path = "/regexword/{entity_id}/events",
    responses(
        (
        status = 200,
...
#[utoipa::path(
    tag = "regexword",
    path = "/regexword/{entity_id}/events/{event_id}",
    responses(
        (
        status = 200,
